...
pub fn review_panel_holds(runs: &[Value], require_commit: Option<&str>) -> Result<(), String> {
    ...
    for (i, run) in runs.iter().enumerate() {
        ...
        if bool_of(fields, "fix_it_failed")
            ...
            || str_field(fields, "verdict")
                .or_else(|| str_field(fields, "Verdict"))
                .as_deref()
                == Some("RequestChanges")
        {
...
fn reviewer_coverage(fields: &Value, include_skips: bool) -> Result<BTreeSet<String>, String> {
    ...
    if include_skips {
        ...
        if let Some(skips) = rubrics.get("reviewers_skipped").and_then(Value::as_array) {
            for skip in skips {
                if str_field(skip, "reason").is_some()
                    && let Some(reviewer) = str_field(skip, "reviewer")
                {
                    reviewers.insert(reviewer);
                }
            }
...
fn replacement_holds(retired: &Value, runs: &[Value]) -> Result<(), String> {
    ...
    if required.is_empty() {
        return Err("chain_merge_ready: superseded run has no model reviewer coverage".to_string());
    }
    ...
    loop {
        let id = str_field(current, "replacement_run_id")
            .or_else(|| str_field(current, "ReplacementRunId"))
            .ok_or("chain_merge_ready: superseded run has no replacement_run_id")?;
        if !seen.insert(id.clone()) {
            return Err("chain_merge_ready: replacement links contain a cycle".to_string());
        }
        let run = runs
            .iter()
            .find(|run| run.get("id").and_then(Value::as_str) == Some(&id))
            .ok_or("chain_merge_ready: replacement run is not attached")?;
        current = run.get("fields").unwrap_or(run);
        let status = str_field(current, "status").or_else(|| str_field(current, "Status"));
        match status.as_deref() {
            Some("Superseded") => continue,
            Some("Recorded") => {
                if !required.is_subset(&reviewer_coverage(current, true)?) {
                    return Err(
                        "chain_merge_ready: replacement does not cover retired reviewers"
                            .to_string(),
                    );
                }
                return Ok(());
            }
            _ => return Err("chain_merge_ready: replacement is not Recorded".to_string()),
        }
    }
}

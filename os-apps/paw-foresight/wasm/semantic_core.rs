...
pub fn plan(nodes: &[Value]) -> Result<Value, String> {
    ...
    let mut active = BTreeSet::new(); let mut visited = BTreeSet::new();
    let mut tasks = vec![]; let mut issues = vec![];
    fn walk(id: &str, depth: usize, by_id: &BTreeMap<String, &Value>, active: &mut BTreeSet<String>, visited: &mut BTreeSet<String>, tasks: &mut Vec<Value>, issues: &mut Vec<Value>) {
        if active.contains(id) { issues.push(json!({"nodeId":id,"result":"cycle_detected","depth":depth})); return; }
        if visited.contains(id) { return; }
        if depth > MAX_DEPTH { issues.push(json!({"nodeId":id,"result":"depth_limit","depth":depth})); return; }
        let Some(node) = by_id.get(id) else { issues.push(json!({"nodeId":id,"result":"missing_prerequisite","depth":depth})); return; };
        active.insert(id.to_owned());
        match parse(field(node,"edges")).ok().and_then(|v|v.as_array().cloned()) {
            Some(edges) => for edge in edges {
                if edge["kind"] == "requires" {
                    if let Some(target) = edge["to_id"].as_str().filter(|s|!s.is_empty()) { walk(target,depth+1,by_id,active,visited,tasks,issues); }
                    else { issues.push(json!({"nodeId":id,"result":"invalid_reference","depth":depth})); }
                }
            },
            None => issues.push(json!({"nodeId":id,"result":"invalid_edges","depth":depth})),
        }
        active.remove(id); visited.insert(id.to_owned());
    ...
    for id in by_id.keys() { walk(id,0,&by_id,&mut active,&mut visited,&mut tasks,&mut issues); }
...
pub fn request(snapshot: &Value, program: &Value) -> Result<Value, String> {
    ...
    let (instructions,criteria) = if task["function"]=="classify_gap" {
        ("Classify the most consequential causal gap in state.node using its actual prerequisites and the supplied world evidence. A future event is a hypothesis, not a false observation. Source references alone do not establish source contents. Do not confuse probability with causal coherence. Return uncertain when the provided evidence cannot distinguish options.",json!({"none":"No specific causal gap identified; this does not establish truth.","timing":"The supplied interval is materially too short.","prerequisite":"A necessary causal prerequisite is missing.","evidence":"The key premise lacks supporting evidence in the supplied input.","uncertain":"Cannot distinguish reliably."}))
    } else {
